...
Use this skill when a developer has a rough idea but is not yet ready for formal specification. This is **pre-spec ideation** — it sits before `sdd-specify` in the workflow and is distinct from `plan`:

- `brainstorm`: surfaces constraints, trade-offs, and risks BEFORE the developer knows what to build.
- `plan`: structures work for something already decided.
...
- the work is already scoped and planned → use `plan` or `parallel-impl`;
...
5. **Recommended next step** — usually: invoke `/flow:sdd-specify` with this brief as context.
...
- `/flow:sdd-specify` — the most common next step;
- `/flow:plan` — if the developer wants to plan before specifying;
...
Recommended next step:
  Invoke /flow:sdd-specify with this brief as context, focusing on
