...
This skill is for planning and orchestration, not direct implementation. It may optionally compose with the SDD skills (`flow:sdd-specify`, `flow:sdd-plan`, `flow:sdd-tasks`), but it must fall back cleanly when SDD is unavailable.
...
1. **Project config**
   ...
   Read the `planner`, `reviewer`, and `scout` keys directly. If a key is absent,
   fall back to the baked-in default for that role without prompting.
...
|---------------|---------|--------------------|
| Copilot CLI   | Planner | `claude-opus-4.7`  |
| Copilot CLI   | Reviewer| `gpt-5.4`          |
| Copilot CLI   | Scout   | `claude-haiku-4.5` |
| Claude Code   | Planner | `claude-opus-4.7`  |
...
During discovery, the scout checks for previously captured knowledge artifacts that relate to the current task — for example, past debugging lessons or resolution patterns stored via `knowledge-save` (or an equivalent repository knowledge sink), as well as architectural decisions or ADRs stored in `arch-review` or another architecture-decision sink.
...
Relevant files: plugin.json, .github/plugin/marketplace.json, skills/plan/SKILL.md
...
Recommended next action: invoke /flow:parallel-impl for implementation
