...
| `parallel-implementation-loop` | There are multiple independent ready tasks and you want disciplined parallel tracks on isolated external worktrees, with TDD and concise design-quality expectations kept explicit and the batch carried through integration, readiness, and PR publication by default. | Boundaries are unclear or tasks interact heavily. | human review on the created or updated PR |
